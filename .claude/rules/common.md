...
日本語のテキスト (i18n 翻訳ファイル / Markdown ドキュメント / コードコメント / コミットメッセージなど) を書くときは、以下のスタイルに従ってください。

### 丸括弧の表記

- **半角丸括弧 (`(`, `)`) を使用する**: 全角丸括弧 (`（`, `）`) は使用しない
- **半角丸括弧の両端には半角スペースを設ける**: 例: `テスト (テスト) テスト`
- **行頭・行末の括弧では行外側のスペースは不要**: 行末の `)` の後ろや、行頭の `(` の前にスペースを入れる必要はない

**良い例**:

- 半角英数字とアンダースコアのみ (最大 20 文字)
- ユーザー ID を取得 (削除済みユーザーは 0 を返す)
- フェーズ 8-1 (i18n キー命名規則)

**悪い例**:

- 半角英数字とアンダースコアのみ（最大 20 文字） <!-- 全角丸括弧を使用している -->
- ユーザー ID を取得(削除済みユーザーは 0 を返す) <!-- 半角丸括弧の両端にスペースがない -->
- フェーズ 8-1（i18n キー命名規則） <!-- 全角丸括弧かつスペースなし -->

**理由**:

- **検索 / grep の安定性**: 全角と半角が混在すると同じテキストでも検索できなくなる場合がある
- **視覚的整列**: 半角に統一して両端にスペースを設けると、日本語と英語混在テキストの可読性が向上する
- **コードとの整合性**: コード内のコメントは元々半角だが、ドキュメントや i18n テキストでも揃えることでツール横断での扱いが統一される

**対象外**:

- コード構文自体 (関数呼び出しの `()` や Go の if 条件式の括弧など) はこのスタイルの対象外
