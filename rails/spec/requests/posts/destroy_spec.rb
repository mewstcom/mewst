...
RSpec.describe "DELETE /posts/:post_id", type: :request do
  ...
  it "他のユーザーのポストを削除しようとした場合、削除されずホームにリダイレクトされること" do
    viewer = FactoryBot.create(:actor)
    ...
    expect(response).to have_http_status(:see_other)
    expect(response).to redirect_to(home_path)
    expect(flash[:notice]).not_to eq("投稿を削除しました")
    expect(PostRecord.kept.exists?(id: post.id)).to be(true)
  end

  it "存在しないポストIDで削除しようとした場合、ホームにリダイレクトされること" do
    viewer = FactoryBot.create(:actor)

    sign_in viewer
    delete "/posts/nonexistent_id"
  ...
  it "既に削除されたポストを削除しようとした場合、ホームにリダイレクトされること" do
    viewer = FactoryBot.create(:actor)
    post = FactoryBot.create(:post_record, profile_record: viewer.profile_record)
    post.discard!
