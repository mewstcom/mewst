...
RSpec.describe CreatePostUseCase do
  ...
  let!(:home_timeline) { instance_spy(ProfileRecord::HomeTimeline) }

  before do
    create(:oauth_application, :mewst_web)

    allow(viewer.profile_record).to receive(:home_timeline).and_return(home_timeline)
  ...
  it "投稿が作れること" do
    ...
    expect(viewer.profile_record.last_post_at).to eq(post.published_at)
