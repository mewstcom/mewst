...
// add_river_v7_notification_outbox_and_cleanup migration header: generate clean
// SQL with `river migrate-get --line main --version N --up/--down` (CLI pinned
// to the go.mod River version, version 1 excluded), add it as a new dbmate
// migration that appends `INSERT INTO river_migration (line, version) VALUES
// ('main', N);` (DELETE on down), then bump this constant to N.
//
...
//
// 更新するときは、add_river_v7_notification_outbox_and_cleanup マイグレーションの
// ヘッダーに記した再現手順に従う: `river migrate-get --line main --version N
// --up/--down` (go.mod の River バージョンに固定した CLI、version 1 は除外) で
// clean SQL を生成し、末尾に `INSERT INTO river_migration (line, version) VALUES
// ('main', N);` を追記する (down では DELETE) 新しい dbmate マイグレーションを追加
// し、この定数を N に更新する。
