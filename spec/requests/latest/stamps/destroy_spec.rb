...
RSpec.describe "DELETE /latest/posts/:post_id/stamp", type: :request, api_version: :latest do
  context "when valid input data" do
    ...
    let!(:post_input) { CreateCommentPostService::Input.from_latest_form(form: comment_post_form) }
    let!(:post) { CreateCommentPostService.new.call(input: post_input).post }
    ...
    it "responses 200" do
      ...
      expected = {
        post: {
          ...
          kind: "comment_post",
          postable: {
            comment: post.comment_post.comment
          },
          profile: {
