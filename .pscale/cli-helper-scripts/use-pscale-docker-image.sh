# Original:
# https://github.com/jonico/pscale-workflow-helper-scripts/blob/4ceba75919d259abc513fcf38285fcca3f3e6abb/.pscale/cli-helper-scripts/use-pscale-docker-image.sh
...
function pscale {
    ...
    if [ -n "$NO_DOCKER" ]; then
        ...
        command="docker run -e PLANETSCALE_SERVICE_TOKEN=${MEWST_PLANETSCALE_SERVICE_TOKEN:-""} -e PLANETSCALE_SERVICE_TOKEN_ID=$MEWST_PLANETSCALE_SERVICE_TOKEN_ID -e HOME=/tmp -v $HOME/.config/planetscale:/tmp/.config/planetscale -e PSCALE_ALLOW_NONINTERACTIVE_SHELL=true --user $(id -u):$(id -g) --rm -i $tty planetscale/pscale:${PSCALE_VERSION:-"latest"} $@"
    fi
