...
const listExportPostMonthsByExportID = `-- name: ListExportPostMonthsByExportID :many
WITH months AS (
    SELECT
        date_trunc('month', (published_at AT TIME ZONE 'UTC') AT TIME ZONE $1::text) AS local_month_start,
...
type ListExportPostMonthsByExportIDParams struct {
	TimeZone string    `db:"time_zone"`
	ExportID uuid.UUID `db:"export_id"`
}

type ListExportPostMonthsByExportIDRow struct {
	LocalMonthStart time.Time `db:"local_month_start"`
...
func (q *Queries) ListExportPostMonthsByExportID(ctx context.Context, arg ListExportPostMonthsByExportIDParams) ([]ListExportPostMonthsByExportIDRow, error) {
	rows, err := q.db.QueryContext(ctx, listExportPostMonthsByExportID, arg.TimeZone, arg.ExportID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListExportPostMonthsByExportIDRow{}
	for rows.Next() {
		var i ListExportPostMonthsByExportIDRow
		if err := rows.Scan(
			&i.LocalMonthStart,
			&i.StartsAt,
			&i.EndsAt,
...
const listExportPostsByExportIDInRange = `-- name: ListExportPostsByExportIDInRange :many
SELECT post_id, content, published_at FROM export_posts
WHERE export_id = $1
  AND published_at >= $2
  AND published_at < $3
  AND date_trunc('month', (published_at AT TIME ZONE 'UTC') AT TIME ZONE $4::text) = $5::timestamp
  AND (published_at, post_id) > ($6::timestamp, $7::uuid)
ORDER BY published_at ASC, post_id ASC
LIMIT $8
`

type ListExportPostsByExportIDInRangeParams struct {
	ExportID         uuid.UUID `db:"export_id"`
	StartsAt         time.Time `db:"starts_at"`
	EndsAt           time.Time `db:"ends_at"`
	TimeZone         string    `db:"time_zone"`
	LocalMonthStart  time.Time `db:"local_month_start"`
	AfterPublishedAt time.Time `db:"after_published_at"`
	AfterID          uuid.UUID `db:"after_id"`
	PageSize         int32     `db:"page_size"`
}

type ListExportPostsByExportIDInRangeRow struct {
...
func (q *Queries) ListExportPostsByExportIDInRange(ctx context.Context, arg ListExportPostsByExportIDInRangeParams) ([]ListExportPostsByExportIDInRangeRow, error) {
	...
	defer rows.Close()
	items := []ListExportPostsByExportIDInRangeRow{}
	for rows.Next() {
		var i ListExportPostsByExportIDInRangeRow
		if err := rows.Scan(&i.PostID, &i.Content, &i.PublishedAt); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
