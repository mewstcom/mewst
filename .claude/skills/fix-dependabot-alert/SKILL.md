...
6. コミット作成
7. Alert 情報・対応内容・検証結果を一時ドキュメントにまとめる
8. **`/summarize-dependency-diff` スキルに委譲して old → new バージョン間の実差分を一時ドキュメントに追記**
9. 一時ドキュメントを `/review` スキルに渡す
10. `/review` のレビュー結果に基づいてマージ可否を判断し報告する
...
- **バージョン間の実差分を確認する**: SemVer の表面的な情報だけで「パッチだから安全」と判断しない。アップデート後に `/summarize-dependency-diff` スキルを呼び出して CHANGELOG / リリースノートを取得し、破壊的変更・追加 CVE の有無を確認する
...
**first_patched_version チェック**: 修正バージョンが存在しない場合は自動アップデートが不可能なため、その旨を一時ドキュメントに記載してレビューに進む(ステップ 9 へ)。アップデート手順(ステップ 5〜8)とバージョン間差分の取得(ステップ 10)はスキップする。
...
- 検証(ステップ 7)・コミット(ステップ 8)・バージョン間差分の取得(ステップ 10) はスキップする
...
<!--
ステップ 10 で `/summarize-dependency-diff` を呼び出した場合、
この後に `## バージョン間差分サマリー` セクションが追記される。
parent パッケージもアップデートしている場合は対象パッケージと parent の
両方について `## バージョン間差分サマリー` セクションが繰り返し追記される。
-->
```

この一時ドキュメントを `/review` スキルに参考ドキュメントとして渡すことで、レビュー実行時に Alert 情報・CVE・対応方針・検証結果に加えて、ステップ 10 で `/summarize-dependency-diff` が追記する「バージョン間差分サマリー」（CHANGELOG・追加 CVE・Breaking change・推移的依存の変化）まで考慮したレビューが可能になる。`/review` 側ではこのサマリーの「Breaking change / 挙動変更」テーブルの各項目について、コードベースで `Grep` による影響範囲確認を行う。

### ステップ 10: バージョン間差分の取得

`/summarize-dependency-diff` スキルに委譲して、old → new バージョン間の実差分（CHANGELOG / 追加 CVE / Breaking change / 推移的依存の変化）をステップ 9 で作成した一時ドキュメントに追記してもらう。

> このステップを省略すると、メジャーアップを跨ぐ更新（例: `erb 5.x → 6.x`）や、パッチアップデートに紛れた behavior change を見落とすリスクがある。Dependabot alert はセキュリティ修正だが、修正バージョンに含まれる他の変更まで安全であるとは限らない。

#### 呼び出し方

Skill ツール経由で `/summarize-dependency-diff` を呼び出し、以下の引数を渡す:

```
<ecosystem> <package> <old_version> <new_version> tmp/fix-dependabot-alert/alert-<番号>.md
```

- `ecosystem`: ステップ 4 で判定した ecosystem（`rubygems` / `npm` / `gomod` / `pip` / `composer`）
- `package`: alert 情報の `dependency.package.name`
- `old_version`: アップデート前のバージョン（ステップ 6 の更新前にロックファイルから読み取っておく）
- `new_version`: アップデート後のロックファイルから読み取った実バージョン（必ず `first_patched_version` 以上）
- 出力先: ステップ 9 で作成した一時ドキュメントと同じパス

#### parent パッケージもアップデートした場合

ステップ 6-B で parent パッケージも更新している場合、対象パッケージ（alert の `package.name`）と parent パッケージの両方について `/summarize-dependency-diff` を呼び出す。parent の挙動変更も影響範囲調査の対象になる。

#### スキルの結果

`/summarize-dependency-diff` は一時ドキュメントの末尾に `## バージョン間差分サマリー` セクションを追記する。差分取得不能だった場合でもサマリー（差分取得不能セクション）が追記され、スキル全体は止めずにステップ 11 に進む。

### ステップ 11: `/review` スキルの呼び出し

Skill ツール経由で `/review` を呼び出し、以下の引数を渡す:

- 比較対象ブランチ名: ステップ 2 で記録した「元のブランチ名」
- 参考ドキュメントのパス: ステップ 9 で作成し、ステップ 10 で「バージョン間差分サマリー」が追記された `tmp/fix-dependabot-alert/alert-<番号>.md`
...
**自動アップデート不能で終了した場合**: 変更がないため `/review` の呼び出しはスキップし、ステップ 13 の報告だけを行う。
...
**「破壊的変更なし」の判定基準**: alert 情報の summary だけでは不十分。**ステップ 10 で `/summarize-dependency-diff` が出力した「バージョン間差分サマリー」（CHANGELOG / 追加 CVE / Breaking change / 推移的依存の変化）** と、`/review` 側で行ったコードベースへの影響範囲調査（Grep）の両方を根拠とする。差分サマリーを確認せずに「パッチアップデートだから安全」と判断してはいけない。
...
- `/summarize-dependency-diff` が「差分取得不能」を出した場合は判断材料が不足しているとみなし、`/review` 側のコードベース調査に重きを置く。それでも判断材料が乏しい場合は Comment で報告してユーザーに委ねる
